...
import { deleteDocument, getDocuments } from "@insurahub/lib/api";
import { Button, IconButton, SelectBox } from "@insurahub/ui";
...
const ClientPage: NextPageWithLayout = (props: any) => {
      ...
      <div className="px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-3 mb-12 flex flex-wrap items-center justify-start gap-x-4 gap-y-4 md:justify-end">
          ...
          <div className="block w-fit pt-5">
            {filteredDocuments.length != 1 ? filteredDocuments.length + " Clients" : "1 Client"}
          </div>
        </div>
                          ...
                          {document.Recipient.map((item: any) => (
                            <div key={item.id}>
                              {item.sendStatus === "NOT_SENT" ? (
                                <span
                                  id="sent_icon"
                                  className="inline-flex h-6 flex-shrink-0 items-center rounded-full bg-blue-100 px-2 py-0.5 text-xs font-medium text-blue-800">
                              ...
                              {item.signingStatus === "SIGNED" ? (
                                <span id="signed_icon">
                                  <span className="inline-flex h-6 flex-shrink-0 items-center rounded-full bg-blue-100 px-2 py-0.5 text-xs font-medium text-blue-800">
                                    {item.email}
                                  </span>
                                </span>
                              ) : (
                        ...
                        <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                          <div>
                            <IconButton
                              ...
                              disabled={document.status === "ADDED"}
                            />
                            <IconButton
                              icon={TrashIcon}
                              onClick={(event: any) => {
