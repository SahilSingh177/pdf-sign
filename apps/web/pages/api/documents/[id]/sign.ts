...
import { sendSigningDoneMail } from "@insurahub/lib/mail";
import { getDocument } from "@insurahub/lib/query";
import { defaultHandler, defaultResponder } from "@insurahub/lib/server";
import { insertImageInPDF, insertTextInPDF } from "@insurahub/pdf";
...
async function postHandler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (unsignedRecipients.length === 0) {
    ...
    for (const signer of signedRecipients) {
      console.log("Sending mail to signer", signer);
      // Wait for 4 seconds;
